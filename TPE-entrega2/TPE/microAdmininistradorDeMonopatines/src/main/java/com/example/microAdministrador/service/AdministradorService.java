...
public class AdministradorService {
	...
	public TarifaResponseDTO establecerNuevaTarifa(long tarifa, int dia, int mes, int anio) {
		...
		this.tarifaFeign.nuevaTarifa(newTarifa);
	...
	public TarifaResponseDTO establecerNuevaTarifaExtra(long tarifa, int dia, int mes, int anio) {
		...
		this.tarifaFeign.nuevaTarifa(newTarifaExtra);
		List<Tarifa> tarifas=this.tarifaFeign.getAllTarifas().getBody();
	...
	public Long getFacturadoByFecha(int ini, int fin, int anio) {
		...
		Long tarifaTotal=(minutosViajes)*(this.tarifaFeign.getTarifa(LocalDate.now()).getBody().getPrecio());
		return tarifaTotal;
	}
	...
	public void editarTarifa(int tarifa) {
		Tarifa t=this.tarifaFeign.getTarifa(LocalDate.now()).getBody();			
		this.tarifaFeign.editTarifa(t.getId(), tarifa);
	}
	
	@Transactional
	public void editarTarifaExtra(int tarifaExtra) {
		List<Tarifa> extras=this.adminRep.getAllTarifasExtra();
    	List<Tarifa> tarifasValidas = extras.stream()
            .filter(tarifa -> !tarifa.getFechaInicio().isAfter(LocalDate.now()))
            .sorted(Comparator.comparing(tarifa -> tarifa.getFechaInicio()))
            .collect(Collectors.toList());
		
    	if(!tarifasValidas.isEmpty()) {
    		Tarifa extraVigente=tarifasValidas.get(tarifasValidas.size()-1);
    		if(extraVigente!=null) {			
    			ResponseEntity<Tarifa> t=this.tarifaFeign.getTarifaById(extraVigente.getId());
    			if(t.getStatusCode().is2xxSuccessful()) {
    				Tarifa tarifa=t.getBody();
    				this.tarifaFeign.editTarifa(tarifa.getId(), tarifaExtra);
    			}
    		}
    	}    	
	}
	...
	public Tarifa getTarifaVigente() {
		ResponseEntity<Tarifa> tarifa=this.tarifaFeign.getTarifa(LocalDate.now());
		return tarifa.getBody(); 
	}
